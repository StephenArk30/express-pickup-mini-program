from flask import Flask, render_template, redirect, url_for, session, request, g, json
...
def get_user_id():
    ...
    secret = '6fe6b1ac666adfd7aafb4862512f002e'  # 填写微信小程序secret
...
def take_express():
    ...
    express.time_take = time.strftime("%H:%M:%S", time.localtime())
    express.state = 1
...
def add_express():
    if request.method == 'GET':
        ...
        express = Express(owner_id=owner_id,
                          ...
                          time_publish=time.strftime("%H:%M:%S", time.localtime()),
                          state=0
                          )
        user.expresses.append(express)
